...
local function updateDisplayValues(k)
    local kelvin = math.celsiusToKelvin(tonumber(BJI_Env.Data[k]) or 0)
    W.tempValues[k] = {
        BJI_Env.Data[k],
...
local function body()
    ...
    if BeginTable("BJIEnvTemperature", {
        ...
        TableNextColumn()
        if IconButton("controlTemperature", BJI_Env.Data.useTempCurve and
                BJI.Utils.Icon.ICONS.check_circle or BJI.Utils.Icon.ICONS.cancel,
                { btnStyle = BJI_Env.Data.useTempCurve and BJI.Utils.Style.BTN_PRESETS.SUCCESS or
                    BJI.Utils.Style.BTN_PRESETS.ERROR, bgLess = true }) then
            BJI_Env.Data.useTempCurve = not BJI_Env.Data.useTempCurve
        end
        ...
        Table({ "tempCurveNoon", "tempCurveDusk", "tempCurveMidnight", "tempCurveDawn" }):forEach(function(k)
            ...
            Text(W.labels[k])
            TableNextColumn()
            nextValue = SliderFloatPrecision(k, BJI_Env.Data[k], ranges.temperature.min, ranges.temperature.max,
                {
                    disabled = not BJI_Env.Data.useTempCurve,
