...
local function updateLabels()
    W.labels.infected = BJI.Managers.Lang.get("infected.play.infected") .. " :"
    W.labels.survivors = BJI.Managers.Lang.get("infected.play.survivors") .. " :"
    W.labels.infectedAmount = string.var("({1})", { BJI.Managers.Lang.get("infected.play.infectedAmount") })

    W.labels.preparationTimeoutAboutToEnd = BJI.Managers.Lang.get("infected.play.preparationTimeoutAboutToEnd")
    W.labels.preparationTimeoutIn = BJI.Managers.Lang.get("infected.play.preparationTimeoutIn")
    W.labels.configChoose = BJI.Managers.Lang.get("infected.play.configChoose") .. " :"
    W.labels.readyMark = string.var("({1})", { BJI.Managers.Lang.get("infected.play.readyMark") })
    W.labels.notReadyMark = string.var("({1})", { BJI.Managers.Lang.get("infected.play.notReadyMark") })

    W.labels.playStartIn = BJI.Managers.Lang.get("infected.play.startIn")
    W.labels.flashStartInfected = BJI.Managers.Lang.get("infected.play.flashInfectedStart")
    W.labels.flashStartSurvivor = BJI.Managers.Lang.get("infected.play.flashSurvivorStart")
    ...
    W.labels.buttons.setAsInfected = BJI.Managers.Lang.get("infected.play.setAsInfected")
end
...
local function updateCache(ctxt)
    ...
    local participant = W.scenario.participants[BJI.Managers.Context.User.playerID]
...
local function onLoad()
    W.scenario = BJI.Managers.Scenario.get(BJI.Managers.Scenario.TYPES.INFECTED)
...
local function drawHeaderPreparation(ctxt)
    ...
    if W.cache.showPreparationActions then
        if IconButton("joinParticipants", W.cache.isParticipant and
            ...
            BJI.Tx.scenario.InfectedUpdate(W.scenario.CLIENT_EVENTS.JOIN)
        end
        ...
        if ctxt.isOwner and not W.cache.isReady then
            ...
            if IconButton("readyInfected", BJI.Utils.Icon.ICONS.check,
                ...
                W.cache.disabledButtons = true -- api request protection
                BJI.Tx.scenario.InfectedUpdate(W.scenario.CLIENT_EVENTS.READY, ctxt.veh.gameVehicleID)
            end
        ...
        if BeginTable("InfectedGameHeader", {
            ...
            if W.cache.showGameActions then
                if IconButton("leaveInfected", BJI.Utils.Icon.ICONS.exit_to_app,
                    ...
                    BJI.Tx.scenario.InfectedUpdate(W.scenario.CLIENT_EVENTS.LEAVE)
                end
                    ...
                    if W.cache.showPreparation then
                        ...
                        if i == 2 and W.cache.showInfectedAssignBtn then
                            ...
                            if Button("forceFugitive" .. p.playerName, W.labels.buttons.setAsInfected,
                                ...
                                W.cache.disabledButtons = true
                                BJI.Tx.scenario.InfectedForceInfected(p.playerID)
                            end
                        end
...
W.body = drawBody
W.getState = function()
    return BJI.Managers.Scenario.is(BJI.Managers.Scenario.TYPES.INFECTED)
end
