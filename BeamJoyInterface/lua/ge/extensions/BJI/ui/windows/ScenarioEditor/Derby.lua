...
local function reloadMarkers(indexArena)
    ...
    local waypoints = W.cache.arenas[indexArena].startPositions:map(function(sp, i)
        return {
            name = BJI_Lang.get("derby.edit.startPositionName"):var({ index = i }),
            pos = sp.pos,
            rot = sp.rot,
            radius = 2,
    ...
    if W.cache.arenas[indexArena].centerPosition then
        ...
        if not name or #name:trim() == 0 then
            name = BJI_Lang.get("derby.edit.centerPosition")
        end
        waypoints:insert({
            name = name,
    ...
    BJI_WaypointEdit.setWaypoints(waypoints)
    W.markersArena = indexArena
end

local function updateLabels()
    W.labels.title = BJI_Lang.get("derby.edit.title")
    W.labels.arena = BJI_Lang.get("derby.edit.arena")
    W.labels.name = BJI_Lang.get("derby.edit.name")
    W.labels.enabled = BJI_Lang.get("derby.edit.enabled")
    W.labels.enabledTooltip = BJI_Lang.get("derby.edit.enabledTooltip")
    W.labels.previewPosition = BJI_Lang.get("derby.edit.previewPosition")
    W.labels.previewPositionTooltip = BJI_Lang.get("derby.edit.previewPositionTooltip")
    W.labels.centerPosition = BJI_Lang.get("derby.edit.centerPosition")
    W.labels.radius = BJI_Lang.get("derby.edit.radius")
    W.labels.startPositions = BJI_Lang.get("derby.edit.startPositions")
    W.labels.amountStartPositionsNeeded = BJI_Lang.get("derby.edit.amountStartPositionsNeeded")
    W.labels.startPositionName = BJI_Lang.get("derby.edit.startPositionName")

    W.labels.buttons.addArena = BJI_Lang.get("derby.edit.buttons.addArena")
    W.labels.buttons.showArena = BJI_Lang.get("derby.edit.buttons.showArena")
    W.labels.buttons.deleteArena = BJI_Lang.get("derby.edit.buttons.deleteArena")
    W.labels.buttons.toggleArenaVisibility = BJI_Lang.get("derby.edit.buttons.toggleArenaVisibility")
    W.labels.buttons.setPreviewPositionHere = BJI_Lang.get("derby.edit.buttons.setPreviewPositionHere")
    W.labels.buttons.showPreviewPosition = BJI_Lang.get("derby.edit.buttons.showPreviewPosition")
    W.labels.buttons.setCenterPositionHere = BJI_Lang.get("derby.edit.buttons.setCenterPositionHere")
    W.labels.buttons.showCenterPosition = BJI_Lang.get("derby.edit.buttons.showCenterPosition")
    W.labels.buttons.addStartPositionHere = BJI_Lang.get("derby.edit.buttons.addStartPositionHere")
    W.labels.buttons.showStartPosition = BJI_Lang.get("derby.edit.buttons.showStartPosition")
    W.labels.buttons.setStartPositionHere = BJI_Lang.get("derby.edit.buttons.setStartPositionHere")
    W.labels.buttons.deleteStartPosition = BJI_Lang.get("derby.edit.buttons.deleteStartPosition")
...
local function onLoad()
    ...
    listeners:insert(BJI_Events.addListener(BJI_Events.EVENTS.CACHE_LOADED,
        function(_, data)
            if data.cache == BJI_Cache.CACHES.DERBY_DATA then
                updateCache()
            end
        end, W.name))
...
local function save()
    ...
    ---@param a BJArena
    BJI_Tx_scenario.DerbySave(W.cache.arenas:map(function(a)
        return {
            name = a.name,
        ...
        if result then
            ...
            BJI_Toast.error(BJI_Lang.get("derby.edit.saveErrorToast"))
        end
...
local function drawArena(ctxt, iArena, arena)
    if BeginTable("BJIScenarioEditorDerbyArena" .. tostring(iArena), {
        ...
        if IconButton("setArenaPreviewPos" .. tostring(iArena), arena.previewPosition and
            ...
            arena.previewPosition = BJI_Cam.getPositionRotation(true)
            W.changed = true
            validateArenas()
        end
        ...
        if arena.previewPosition then
            ...
            if IconButton("goToArenaPreviewPosition" .. tostring(iArena), BJI.Utils.Icon.ICONS.pin_drop) then
                ...
                BJI_Cam.setPositionRotation(arena.previewPosition.pos, arena.previewPosition.rot)
            end
        ...
        if IconButton("setArenaCenterPos" .. tostring(iArena), arena.centerPosition and
                ...
                { btnStyle = arena.centerPosition and BJI.Utils.Style.BTN_PRESETS.WARNING or
                    BJI.Utils.Style.BTN_PRESETS.SUCCESS, disabled = W.cache.disableButtons or
                    ctxt.camera ~= BJI_Cam.CAMERAS.FREE }) then
            arena.centerPosition = BJI_Cam.getPositionRotation().pos
            W.changed = true
            validateArenas()
            reloadMarkers(iArena)
        end
        ...
        if arena.centerPosition then
            ...
            if IconButton("goToArenaCenterPosition" .. tostring(iArena), BJI.Utils.Icon.ICONS.pin_drop) then
                ...
                BJI_Cam.setPositionRotation(arena.centerPosition)
            end
        ...
        arena.startPositions:forEach(function(sp, i)
            ...
            if IconButton("goToArenaStartPos" .. tostring(iArena) .. tostring(i), BJI.Utils.Icon.ICONS.pin_drop) then
                if ctxt.isOwner then
                    BJI_Veh.setPositionRotation(arena.startPositions[i].pos, arena.startPositions[i].rot,
                        { safe = false })
                    if ctxt.camera == BJI_Cam.CAMERAS.FREE then
                        BJI_Cam.toggleFreeCam()
                    end
                    ...
                    BJI_Cam.setCamera(BJI_Cam.CAMERAS.FREE)
                    BJI_Cam.setPositionRotation(sp.pos + vec3(0, 0, 1), sp.rot * quat(0, 0, 1, 0))
                end
            end
