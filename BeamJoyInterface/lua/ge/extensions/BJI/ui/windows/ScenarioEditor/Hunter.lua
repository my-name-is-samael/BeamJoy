...
local function drawWaypoints(ctxt)
    W.cache.targets:forEach(function(waypoint, i)
        ...
        Indent(1)
        LineBuilder():text(W.labels.radius)
            :inputNumeric({
...
local function body(ctxt)
    AccordionBuilder()
        :label(BJI.Managers.Lang.get("hunter.edit.hunters"))
        :commonStart(function(isOpen)
    ...
    AccordionBuilder()
        :label(BJI.Managers.Lang.get("hunter.edit.hunted"))
        :commonStart(function(isOpen)
    ...
    AccordionBuilder()
        :label(BJI.Managers.Lang.get("hunter.edit.waypoints"))
        :commonStart(function(isOpen)
...
local function open()
    ...
    W.cache.targets = Table(hunterData.targets)
        :map(function(target)
    ...
    W.cache.hunterPositions = Table(hunterData.hunterPositions)
        :map(function(hunter)
            return math.tryParsePosRot(hunter)
        end)
    W.cache.huntedPositions = Table(hunterData.huntedPositions)
        :map(function(hunted)
