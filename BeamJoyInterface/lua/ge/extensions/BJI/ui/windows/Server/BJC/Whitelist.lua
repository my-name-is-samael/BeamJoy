...
local function drawWhitelistOnlinePlayers(playerNames, labels, cache)
    ...
    if Button("addAllConnectedPlayers", labels.whitelist.addAllConnectedPlayers,
        ...
        playerNames:forEach(function(playerName)
            if not table.includes(BJI_Context.BJC.Whitelist.PlayerNames, playerName) then
                BJI_Tx_moderation.whitelist(playerName)
            end
        end)
    end
    Indent()
    playerNames:forEach(function(playerName)
        included = table.includes(BJI_Context.BJC.Whitelist.PlayerNames, playerName)
...
return function(ctxt, labels, cache)
    ...
    if cache.whitelist.canToggleState then
        if IconButton("toggleWhitelist", BJI_Context.BJC.Whitelist.Enabled and BJI.Utils.Icon.ICONS.check_circle or
                BJI.Utils.Icon.ICONS.cancel, { bgLess = true, btnStyle = BJI_Context.BJC.Whitelist.Enabled and
                    BJI.Utils.Style.BTN_PRESETS.SUCCESS or BJI.Utils.Style.BTN_PRESETS.ERROR, disabled = cache.disableInputs }) then
            cache.disableInputs = true
            BJI_Tx_config.bjc("Whitelist.Enabled", not BJI_Context.BJC.Whitelist.Enabled)
            BJI_Context.BJC.Whitelist.Enabled = not BJI_Context.BJC.Whitelist.Enabled
        end
    else
        Icon(BJI_Context.BJC.Whitelist.Enabled and BJI.Utils.Icon.ICONS.check_circle or
            BJI.Utils.Icon.ICONS.cancel, {
                color = BJI_Context.BJC.Whitelist.Enabled and
                    BJI.Utils.Style.TEXT_COLORS.SUCCESS or BJI.Utils.Style.TEXT_COLORS.ERROR
            })
        TooltipText(BJI_Context.BJC.Whitelist.Enabled and labels.whitelist.enabled or
    ...
    local canAdd = #cache.whitelist.addName > 0 and
        not table.includes(BJI_Context.BJC.Whitelist.PlayerNames, cache.whitelist.addName)
    if IconButton("addWhitelist", BJI.Utils.Icon.ICONS.add, {
        ...
        cache.disableInputs = true
        BJI_Tx_moderation.whitelist(cache.whitelist.addName)
